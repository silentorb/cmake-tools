...
#endif ()
macro(list_contains haystack needle result)
  #  message("${haystack} ${needle}")
  set(${result} 0)
  foreach (item ${haystack})
    #    message ("${item} = ${needle}")
    if ("${item}" STREQUAL "${needle}")
      set(${result} 1)
    endif ()
  endforeach ()
endmacro()

#set(test_list "art;bat")
#list_contains("${test_list}" art test_result)
#message("${test_result}")
#efg()
if (IOS)
...
macro(create_target target is_executable)
  ...
  set(target_is_executable ${is_executable})
  #  set(${target_is_executable} ${is_executable} PARENT_SCOPE)
...
macro(require_hierarchy library_name)
  list_contains("${required_libraries}" ${library_name} already_done)
  #  message("${required_libraries}")
  if (NOT ${already_done})
    if (TARGET ${library_name})
      target_link_libraries(${CURRENT_TARGET} $<TARGET_FILE:${library_name}>)
      #      message(WARNING ":: ${library_name}")
      list(APPEND required_libraries ${library_name})
      #      message("- ${library_name} ${${library_name}_dependencies}")
      foreach (child_library_name ${${library_name}_dependencies})
        require_hierarchy(${child_library_name})
      endforeach ()
      foreach (child_library_name ${${library_name}_external_dependencies})
        list_contains("${required_libraries}" ${child_library_name} already_done)
        if (NOT ${already_done})
          list(APPEND required_libraries ${child_library_name})
          link_external(${child_library_name})
          #          message(WARNING "::: ${child_library_name}")
        endif ()
      endforeach ()
...
macro(require)
  if (LOCAL_TARGET)
    ...
    foreach (library_name ${ARGN})
      ...
      if (TARGET ${library_name})
        ...
        elseif (${target_is_executable})
          require_hierarchy(${library_name})
        else ()
          target_link_libraries(${CURRENT_TARGET} $<TARGET_FILE:${library_name}>)
        endif ()
    ...
    endforeach ()
    set(${CURRENT_TARGET}_dependencies ${ARGN} PARENT_SCOPE)
...
macro(link_external_static path)
  ...
  list(APPEND ${CURRENT_TARGET}_external_dependencies ${path})
  ...
  if (MSVC)
    ...
    endforeach ()
    set(DEPENDENCY_PATH ${MYTHIC_DEPENDENCIES}/Release/${path}/include${include_suffix})
    #    message("${CURRENT_TARGET} ${DEPENDENCY_PATH}")
    ...
    if (IOS)
      ...
      message("* ${CURRENT_TARGET} ${${CURRENT_TARGET}_external_dependencies}")
    endif ()
